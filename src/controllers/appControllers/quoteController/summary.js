...
const summary = async (req, res) => {
  try {
    ...
    // Main aggregation for quote summary
    const result = await Model.aggregate([
      {
        $match: {
          removed: false,
          createdBy: new mongoose.Types.ObjectId(req.admin._id), // Admin-specific restriction
          date: {
    ...
    // Ensuring all statuses are represented
    ...
    // Unpaid aggregation logic
    const unpaid = await Model.aggregate([
      {
        $match: {
          removed: false,
          createdBy: new mongoose.Types.ObjectId(req.admin._id), // Admin-specific restriction
          paymentStatus: { $in: ['unpaid', 'partially'] }, // Adjusted for unpaid quotes
          date: {
            $gte: startDate.toDate(),
            $lte: endDate.toDate(),
          },
    ...
    const finalResult = {
      total,
      total_unpaid: unpaid.length > 0 ? unpaid[0].total_amount : 0,
      type: defaultType,
      performance: result,
    };
